...
//This is only use in Goerli testnet
import "./priceConvertGoerli.sol"; 
...
contract ScholarDetails {
	...
	address PriceContractAdd; //this only relevant for running on goerli testnet to get value in USD
	...
	function viewUSDWEI(uint256 _number) public view returns (uint256) {
		PriceConversion p = PriceConversion(PriceContractAdd);
        uint256 price = p.getUSDWEI(_number);
        return price;
    }
	...
	{	//Convert the amount from USD to wei
		PriceConversion p = PriceConversion(PriceContractAdd);
		uint256 cvrAmt = p.getUSDWEI(_Amount);
